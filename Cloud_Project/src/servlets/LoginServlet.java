...
public class LoginServlet extends HttpServlet {
	...
	private int nnum =0;
	public void displaySAMLInfo(Node node, String parent, HttpServletRequest req, HttpServletResponse res) throws IOException
    {
...
//            out.println("<u>Examining <b>" + parent + nodeName + "</b></u><br>");
            //session.setAttribute("SAML", "After node name");
             ...
             //               System.out.println("Attribute: <b>" + ((org.w3c.dom.Node) attrib).getNodeName() + "</b>: " + ((org.w3c.dom.Node) attrib).getNodeValue()  + "<br>");
                            }
                     }
               }
               ...
               if (null != list)
               {
            	   //session.setAttribute("SAML", "List length after if " + list.getLength());
                    ...
                    //	  	 session.setAttribute("SAML", nnum+1);
                             ...
                             if (list.item(0).getNodeName() == "#text" & list.item(0).getTextContent().toString().contains("@"))
                             {
                                 System.out.println("Text value: <b>" + list.item(0).getTextContent() + "</b><br>");
                 ...
                 //                   System.out.print("<b>" + ((org.w3c.dom.Node) temp).getNodeName() + "</b>");
                                    if (i < nChild - 1)
                                    {
                                        // Separate the names.
                   //                     System.out.print(", ");
                     ...
                     //                   System.out.print(".");
                       ...
                       //          System.out.println("<br>");
                                 ...
                                 {
                                    Node temp = (Node) list.item(i);
                                    displaySAMLInfo(temp, parent + nodeName + "\\", req, res);
                                 }
                           }
                      }
                  }
 ...
 try 
    {
        ...
        session.setAttribute("userx", session.getAttribute("SAML").toString().substring(0, session.getAttribute("SAML").toString().indexOf("@")));
    	}
    	else
    	{
    		session = request.getSession();
    		session.setAttribute("SAML", "test1@test7786.onmicrosoft.com");
    		//session.setAttribute("SAML", "arun@test7786.onmicrosoft.com");
    		
    	}
    	session.setAttribute("userx", session.getAttribute("SAML").toString().split("@")[0]);
		session.setAttribute("tenantId", session.getAttribute("SAML").toString().split("@")[1]);
    	/* Ganesh - Start */
		...
		String uname = session.getAttribute("SAML").toString().substring(0, session.getAttribute("SAML").toString().indexOf("@"));
		session.setAttribute("userName", uname);
		session.setAttribute("UserId", uname);
		//response.sendRedirect("test.jsp");
		try{
		    Class.forName("com.mysql.jdbc.Driver");
		    Connection conn = (Connection) DriverManager.getConnection("jdbc:mysql://robocodedb.cloudapp.net:3306/form","naren","naren");
		    String sqle = "SELECT * FROM Pages where HierarchicalRoleID in ";
		    String sqll = "SELECT HierarchicalRoleId from UserHierarchicalRole Where UserId = '" + session.getAttribute("userName") + "'";
		    PreparedStatement ppl = (PreparedStatement) conn.prepareStatement(sqll);
		    ...
		    System.out.println("roles for the user "+session.getAttribute("userName").toString()+" is"+Arrays.toString(userRoles.toArray()));
		    ...
		    PreparedStatement pps = (PreparedStatement) conn.prepareStatement(sqle);
			...
			if (!(user_DTO.get(0).getUserName().equalsIgnoreCase("Admin")) ) {
				...
				/* Ganesh  - Start */
				 try{
					    ...
					    String sqle = "SELECT * FROM Pages where HierarchicalRoleID in ";
					    String sqll = "SELECT HierarchicalRoleId from UserHierarchicalRole Where UserId = '" + user_DTO.get(0).getUserName() + "'";
					    PreparedStatement ppl = (PreparedStatement) conn.prepareStatement(sqll);
					    ResultSet rl = (ResultSet) ppl.executeQuery();
					    while(rl.next())
					    {
					    ...
					    System.out.println("roles for the user "+user_DTO.get(0).getUserName()+" is"+Arrays.toString(userRoles.toArray()));
...
//						    System.out.println(rs.getString("servicename"));
	...
	private void trecurs(Connection c, String rl) 
	{
		...
		String sqlr = "SELECT * FROM Hierarchy where ParentHierarchicalRoleId = '" + rl + "'" ;
		PreparedStatement ppr = (PreparedStatement) c.prepareStatement(sqlr);
